...
#[inline]
fn vector_times_scalar<B: Builder>(
    builder: &mut B,
    res_type: &'static TypeName,
    res_id: u32,
    vector: u32,
    scalar: u32,
) {
    ...
    builder.push_instruction(Instruction::new(
        ...
        Some(res_type),
        Some(res_id),
        vec![Operand::IdRef(vector), Operand::IdRef(scalar)],
    ));
}
#[inline]
fn matrix_times_scalar<B: Builder>(
    builder: &mut B,
    res_type: &'static TypeName,
    res_id: u32,
    matrix: u32,
    ...
    builder.push_instruction(Instruction::new(
        ...
        Some(res_type),
        Some(res_id),
        vec![Operand::IdRef(matrix), Operand::IdRef(scalar)],
    ));
}

#[inline]
pub(crate) fn multiply<B: Builder>(
    builder: &mut B,
    ...
    let (l_arg, r_arg) = match args.len() {
        2 => (args[0], args[1]),
        n if n > 2 => (multiply(builder, &args[0..n - 1])?, args[n - 1]),
    ...
    let res_type = match (l_type, r_type) {
        ...
        (&Vec(v_len, l_scalar), &Mat(m_len, &Vec(_, r_scalar)))
            if v_len == m_len && l_scalar == r_scalar && l_scalar.is_float() =>
        {
        ...
        (&Mat(m_len, &Vec(_, l_scalar)), &Vec(v_len, r_scalar))
            if v_len == m_len && l_scalar == r_scalar && l_scalar.is_float() =>
        {
        ...
        (&Mat(l_len, &Vec(_, l_scalar)), &Mat(r_len, &Vec(_, r_scalar)))
            if l_len == r_len && l_scalar == r_scalar && l_scalar.is_float() =>
        {
            let res_type = builder.register_type(l_type);
...
pub(crate) fn dot<B: Builder>(
    ...
    match (l_type, r_type) {
        (&Vec(l_size, l_scalar), &Vec(r_size, r_scalar))
            if l_size == r_size && l_scalar == r_scalar && l_scalar.is_float() =>
        {
